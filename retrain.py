...
def add_embedding_retrain_ops(hidden_dim, bottleneck_tensor,
                          quantize_layer):
  """Adds a new fully-connected layer for sentiment embedding.

  We need to retrain the top layer to embed sentiment, so this function
  adds the right operations to the graph, along with some variables to hold the
  weights, and then sets up all the gradients for the backward pass.

  Args:
    hidden_dim: dimension of embedding
    embed_tensor_name: Name string for the new node that produces embeddings.
  ...
  Returns:
    Tensors for the bottleneck input, ground truth input, and embedded results.
  """
  ...
  layer_name = 'embed_ops'
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      initial_value = tf.truncated_normal(
          [bottleneck_tensor_size, hidden_dim], stddev=0.001)
      layer_weights = tf.Variable(initial_value, name='img_embed_weights')
      variable_summaries(layer_weights)

    with tf.name_scope('biases'):
      layer_biases = tf.Variable(tf.zeros([hidden_dim]), name='img_embed_biases')
      variable_summaries(layer_biases)

    with tf.name_scope('Wx_plus_b'):
      embedded = tf.matmul(bottleneck_input, layer_weights) + layer_biases
      tf.summary.histogram('pre_activations', embedded)
  ...
  tf.summary.histogram('activations', embedded)

  return bottleneck_input, ground_truth_input, embedded
...
def embed_image(image_dir, testing_percentage=10, validation_percentage=10, quantize_layer=False):
  ...
  with graph.as_default():
    bottleneck_input, ground_truth_input, embedded = add_embedding_retrain_ops(128, embed_tensor_name, bottleneck_tensor,
                          quantize_layer)

  return graph, bottleneck_input, embedded
